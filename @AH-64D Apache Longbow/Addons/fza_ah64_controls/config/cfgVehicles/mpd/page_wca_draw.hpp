class Static {
    type = line;
    width = 3;
    points[] = {
        //Center lines
        {{0.496, 0.174}, 1}, {{0.496, 0.832}, 1}, {},
        {{0.504, 0.174}, 1}, {{0.504, 0.832}, 1}, {},
        //Curved box
        MPD_POINTS_BOX(Null, (0.5 - MPD_TEXT_WIDTH * 6.5), 0.105, (13 * MPD_TEXT_WIDTH), MPD_TEXT_HEIGHT) 
    };
};

MPD_ARROW_C(ENG, MPD_POS_BUTTON_TB_1_X, MPD_POS_BUTTON_T_Y, 3)
MPD_TEXT_C(ENG, MPD_POS_BUTTON_TB_1_X, MPD_POS_BUTTON_T_Y, MPD_TEXT_STATIC("DTU"))

MPD_ARROW_C(FAULT, MPD_POS_BUTTON_TB_2_X, MPD_POS_BUTTON_T_Y, 5)
MPD_TEXT_C(FAULT, MPD_POS_BUTTON_TB_2_X, MPD_POS_BUTTON_T_Y, MPD_TEXT_STATIC("FAULT"))

MPD_ARROW_C(IBIT, MPD_POS_BUTTON_TB_3_X, MPD_POS_BUTTON_T_Y, 4)
MPD_TEXT_C(IBIT, MPD_POS_BUTTON_TB_3_X, MPD_POS_BUTTON_T_Y, MPD_TEXT_STATIC("IBIT"))

MPD_ARROW_C(VERS, MPD_POS_BUTTON_TB_5_X, MPD_POS_BUTTON_T_Y, 4)
MPD_TEXT_C(VERS, MPD_POS_BUTTON_TB_5_X, MPD_POS_BUTTON_T_Y, MPD_TEXT_STATIC("VERS"))

MPD_ARROW_C(UTIL, MPD_POS_BUTTON_TB_6_X, MPD_POS_BUTTON_T_Y, 5)
MPD_TEXT_C(UTIL, MPD_POS_BUTTON_TB_6_X, MPD_POS_BUTTON_T_Y, MPD_TEXT_STATIC("UTIL"))

MPD_TEXT_C(DMS, MPD_POS_BUTTON_TB_1_X, MPD_POS_BUTTON_B_Y, MPD_TEXT_STATIC("DMS"))

MPD_TEXT_C(RESET, MPD_POS_BUTTON_TB_4_X, MPD_POS_BUTTON_B_Y, MPD_TEXT_STATIC("RESET"))

MPD_ARROW_C(WCA, MPD_POS_BUTTON_TB_6_X, MPD_POS_BUTTON_B_Y, 3)
MPD_TEXT_C(WCA, MPD_POS_BUTTON_TB_6_X, MPD_POS_BUTTON_B_Y, MPD_TEXT_STATIC("WCA"))

MPD_TEXT_C(WCACenter, 0.5, 0.105, MPD_TEXT_STATIC("W/C/A HISTORY"))

#define MPD_WCA_COLUMN_1_X (0.49 - 18 * MPD_TEXT_WIDTH)
#define MPD_WCA_COLUMN_2_X 0.52
#define MPD_WCA_COLUMN_Y 0.17
#define MPD_WCA_COLUMN_SPACING (MPD_TEXT_HEIGHT + 0.015)

#define MPD_WCA_ITEM(userNum, posX, yIndex) class WcaItem##userNum { \
    color[] = {__EVAL("2 - user" + str (MFD_OFFSET + userNum)), MPD_COND_USER(userNum), __EVAL("0.02 * user" + str (MFD_OFFSET + userNum)) + " - 0.02", 1}; \
    MPD_TEXT_R(Text, posX, (MPD_WCA_COLUMN_Y + yIndex * MPD_WCA_COLUMN_SPACING), MPD_TEXT_USER(userNum)) \
};

MPD_WCA_ITEM(0, MPD_WCA_COLUMN_1_X, 0)
MPD_WCA_ITEM(1, MPD_WCA_COLUMN_1_X, 1)
MPD_WCA_ITEM(2, MPD_WCA_COLUMN_1_X, 2)
MPD_WCA_ITEM(3, MPD_WCA_COLUMN_1_X, 3)
MPD_WCA_ITEM(4, MPD_WCA_COLUMN_1_X, 4)
MPD_WCA_ITEM(5, MPD_WCA_COLUMN_1_X, 5)
MPD_WCA_ITEM(6, MPD_WCA_COLUMN_1_X, 6)
MPD_WCA_ITEM(7, MPD_WCA_COLUMN_1_X, 7)
MPD_WCA_ITEM(8, MPD_WCA_COLUMN_1_X, 8)
MPD_WCA_ITEM(9, MPD_WCA_COLUMN_2_X, 0)
MPD_WCA_ITEM(10, MPD_WCA_COLUMN_2_X, 1)
MPD_WCA_ITEM(11, MPD_WCA_COLUMN_2_X, 2)
MPD_WCA_ITEM(12, MPD_WCA_COLUMN_2_X, 3)
MPD_WCA_ITEM(13, MPD_WCA_COLUMN_2_X, 4)
MPD_WCA_ITEM(14, MPD_WCA_COLUMN_2_X, 5)
MPD_WCA_ITEM(15, MPD_WCA_COLUMN_2_X, 6)
MPD_WCA_ITEM(16, MPD_WCA_COLUMN_2_X, 7)
MPD_WCA_ITEM(17, MPD_WCA_COLUMN_2_X, 8)